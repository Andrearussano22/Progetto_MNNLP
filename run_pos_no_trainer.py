...
def parse_args():
    ...
    #adapting dataset labels' to model ones
...
universal_to_custom = {v:k for k,v in custom_to_universal.items()}

    #mapping new labels
def map_custom_to_upos(tag):
    return custom_to_universal.get(tag, "X")
    #mapping EAGLES tag to UPOS tag (models')
...
def main():
    ...
    if args.dataset is not None:
        ...
        # Generating label_list and label_to_id
        label_list = list(model.config.label2id.keys())
        label_to_id = model.config.label2id
        
        
        model.config.label2id = label_to_id
        model.config.id2label = {id_: label for label, id_ in label_to_id.items()}
